...
export default function Sidebar({ isCollapsed = false }: SidebarProps) {
  ...
  // Market overview items with real-time data
  const marketOverview = [
    {
      label: 'S&P 500 (SPY)',
      price: spyData?.price ? formatCurrency(spyData.price) : '$527.12',
      change: spyData?.changePercent ? formatPercentage(spyData.changePercent) : '+0.85%',
      isPositive: spyData?.changePercent ? spyData.changePercent >= 0 : true
    },
    {
      label: 'Bitcoin',
      price: btcData?.price ? formatCurrency(btcData.price) : '$43,567',
      change: btcData?.changePercent ? formatPercentage(btcData.changePercent) : '+2.34%',
      isPositive: btcData?.changePercent ? btcData.changePercent >= 0 : true
    },
    {
      label: 'Ethereum',
      price: ethData?.price ? formatCurrency(ethData.price) : '$2,687',
      change: ethData?.changePercent ? formatPercentage(ethData.changePercent) : '-0.92%',
      isPositive: ethData?.changePercent ? ethData.changePercent >= 0 : false
    }
  ];
  ...
  return (
    <div
      className={classNames(
        'flex flex-col bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 transition-all duration-300 h-screen flex-shrink-0',
    ...
    >
      ...
      <div className="flex items-center h-16 px-4 border-b border-gray-200 dark:border-gray-700">
        ...
        {isCollapsed && (
          <Link 
            to="/" 
            className="flex items-center justify-center w-full hover:opacity-80 transition-opacity cursor-pointer group"
            title="Go to Dashboard"
          >
            <ChartBarIcon className="h-8 w-8 text-blue-600 group-hover:text-blue-700 transition-colors mx-auto" />
          </Link>
        )}
      </div>
              ...
              {marketOverview.map((item, index) => (
                <div key={index} className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <div className="text-xs font-medium text-gray-900 dark:text-white">{item.price}</div>
